...
class PrimStepper:
    ...
    def __init__(self, V, E, start):
        self.V, self.E, self.start = V, E, start
        self.reset()

    def reset(self):
        self.state = PrimStepper.State.INIT_0
    ...
    def next(self):
        ...
        if s == self.State.IF_CHECK:
            ...
            if v not in self.T and self.cost[v] > e.weight:
...
class PrimGUI:
    ...
    def __init__(self, master):
        self.master = master; master.title('Prim Visualization')

        # state markers
        self.u_text = None; self.v_text = None
        self.v_labels = {}
        self.last_edge = None
        self.arrow_items = []
        ...
        self.listbox = tk.Listbox(left, width=100, height=len(self.code_lines), font=('Consolas',14))
        for ln in self.code_lines: self.listbox.insert(tk.END, ln)
        self.listbox.pack()
        ...
        self.stepper = PrimStepper(self.vertices, self.edges,
                                   next(v for v in self.vertices if v.id == 'D'))

        for j,v in enumerate(self.vertices):
            c = tk.Label(tf, text='∞', width=6, relief='sunken', bg='white',
                         font=('Consolas',14), fg='blue')
            c.grid(row=0, column=j+1, padx=5, pady=5); self.cost_labels.append(c)
            p = tk.Label(tf, text='-', width=6, relief='sunken', bg='white',
                         font=('Consolas',14))
            p.grid(row=2, column=j+1, padx=5, pady=5); self.parent_labels.append(p)
        ...
        # canvas drawing
        self.canvas = tk.Canvas(center, width=1200, height=800, bg='white'); self.canvas.pack()
        self.edge_items = {}
        for e in self.edges:
            if e.u.id < e.v.id:
                ln = self.canvas.create_line(e.u.x, e.u.y, e.v.x, e.v.y, fill='gray', width=2)
        ...
        for v in self.vertices:
            oid = self.canvas.create_oval(v.x-self.VERTEX_RADIUS, v.y-self.VERTEX_RADIUS,
        ...
        for v in self.vertices:
            lbl = self.canvas.create_text(v.x+self.VERTEX_RADIUS+30, v.y+self.VERTEX_RADIUS+15,
                                         text='cost', font=('Consolas',16), anchor='w', fill='blue')
            val = self.canvas.create_text(v.x+self.VERTEX_RADIUS+80, v.y+self.VERTEX_RADIUS+15,
                                         text='0' if v==self.stepper.start else '∞',
                                         font=('Consolas',16), anchor='w', fill='blue')
            self.cost_labels_canvas[v] = lbl; self.cost_values_canvas[v] = val
    ...
    def blink_updated_values(self, vertex, update_type):
        self.blinking_items.clear(); self.original_texts.clear()
        idx = self.vertex_indices[vertex]
        if update_type == 'cost':
            self.blinking_items += [(self.cost_labels[idx], 'label'),
        ...
        for item,typ in self.blinking_items:
            txt = item.cget('text') if typ=='label' else self.canvas.itemcget(item,'text')
            self.original_texts.append(txt)
        self.blink_count = 0; self.do_blink()

    def do_blink(self):
        if self.blink_count >= self.BLINK_COUNT*2:
            for (item,typ),txt in zip(self.blinking_items,self.original_texts):
                if typ=='label': item.config(text=txt)
                else: self.canvas.itemconfig(item, text=txt)
            self.update_tables(); return
        for i,(item,typ) in enumerate(self.blinking_items):
            if self.blink_count%2==0:
                if typ=='label': item.config(text='')
                else: self.canvas.itemconfig(item,text='')
            else:
                if typ=='label': item.config(text=self.original_texts[i])
                else: self.canvas.itemconfig(item,text=self.original_texts[i])
        self.blink_count +=1; self.master.after(200,self.do_blink)
    ...
    def highlight_T(self):
        ...
        if len(pts)>=3:
            ...
            pad=self.REGION_PADDING; padded=[]
            for x,y in hull:
                dx,dy=x-cx,y-cy; d=math.hypot(dx,dy)
                nx,ny=(cx+(dx/d)*(d+pad),cy+(dy/d)*(d+pad)) if d else (x,y)
                padded.append((nx,ny))
            coords=[c for p in padded for c in p]
        else:
            xs,ys=zip(*pts); m=self.VERTEX_RADIUS
            coords=[min(xs)-m,min(ys)-m,max(xs)+m,min(ys)-m,max(xs)+m,max(ys)+m,min(xs)-m,max(ys)+m]
        if not self.t_frame:
            self.t_frame=self.canvas.create_polygon(coords,outline='blue',dash=(4,2),fill='',width=5)
    ...
    def on_next(self):
        ...
        # cleanup entering while
        if res.code_line==self.WHILE_LINE:
            if self.v_text: self.canvas.delete(self.v_text); self.v_text=None
            for lid in list(self.v_labels.values()): self.canvas.delete(lid)
            self.v_labels.clear()
        # for-loop prep: grey all edges
        if res.code_line==self.FOR_LINE:
            if self.last_edge: self.canvas.itemconfig(self.last_edge, fill='gray', width=2); self.last_edge=None
            if self.v_text: self.canvas.delete(self.v_text); self.v_text=None
            for lid in list(self.v_labels.values()): self.canvas.delete(lid)
            self.v_labels.clear()
            for key in self.edge_items: self.canvas.itemconfig(self.edge_items[key], fill='gray', width=2)
            self.potential_edges={e for e in self.edges if e.u==self.stepper.u and e.v not in self.stepper.T}
            for e in self.potential_edges:
                ...
                lid=self.canvas.create_text(e.v.x+self.LABEL_OFFSET,e.v.y-self.LABEL_OFFSET,
                                           text='v',font=('Consolas',24),fill='red')
                self.v_labels[e.v]=lid
        # if-check: red highlight
        if res.code_line==self.IF_LINE and res.edge:
            ...
            if res.node not in self.v_labels:
                self.v_text=self.canvas.create_text(res.node.x+self.LABEL_OFFSET,res.node.y-self.LABEL_OFFSET,
                                                    text='v',font=('Consolas',24),fill='red')
                self.v_labels[res.node]=self.v_text
        # cost update blink
        if res.code_line==self.UPDATE_COST_LINE:
            self.update_tables(); self.blink_updated_values(res.node,'cost')
        # parent update blink
        if res.code_line==self.UPDATE_PARENT_LINE:
            self.update_tables(); self.blink_updated_values(res.node,'parent')
        # end-if cleanup
        if res.code_line==self.END_IF_LINE:
            if self.last_edge: self.canvas.itemconfig(self.edge_items[self.last_edge], fill='gray', width=2); self.last_edge=None
            if self.v_text: self.canvas.delete(self.v_text); self.v_text=None
            for lid in list(self.v_labels.values()): self.canvas.delete(lid)
            self.v_labels.clear()
        # add-to-T: draw MST edge
        if res.code_line==self.ADD_TO_T_LINE:
            self.highlight_T()
            node=res.node; p=self.stepper.parent[node]
            if p:
                dx,dy=node.x-p.x,node.y-p.y; d=math.hypot(dx,dy); r=self.VERTEX_RADIUS
        ...
        # mark u
        if self.u_text: self.canvas.delete(self.u_text)
        u=self.stepper.u
        if u:
            self.u_text=self.canvas.create_text(u.x+self.LABEL_OFFSET,u.y-self.LABEL_OFFSET,
                                                text='u',font=('Consolas',24),fill='green')
        # code highlight
        self.listbox.selection_clear(0,tk.END)
        if 0<=res.code_line<len(self.code_lines):
            self.listbox.selection_set(res.code_line); self.listbox.see(res.code_line)
        v_id=res.node.id if (res.node and res.node is not u) else '-'
        self.status_label.config(text=f"u = {u.id if u else '-'},  v = {v_id}")

    def on_reset(self):
        for arr in self.arrow_items: self.canvas.delete(arr)
        self.arrow_items.clear()
        if self.t_frame: self.canvas.delete(self.t_frame); self.t_frame=None
        if self.u_text: self.canvas.delete(self.u_text); self.u_text=None
        if self.v_text: self.canvas.delete(self.v_text); self.v_text=None
        for lid in self.v_labels.values(): self.canvas.delete(lid)
        self.v_labels.clear()
        self.last_edge=None
        self.stepper.reset()
        self.potential_edges=set()
        for key in self.edge_items: self.canvas.itemconfig(self.edge_items[key], fill='gray', width=2)
        for v,oid in self.node_items.items(): self.canvas.itemconfig(oid, fill='white', outline='black', width=3)
        for v in self.vertices:
            default='0' if v==self.stepper.start else '∞'
            self.canvas.itemconfig(self.cost_values_canvas[v], text=default)
        self.listbox.selection_clear(0,tk.END)
        self.status_label.config(text='u = -,  v = -')
        self.update_tables(); self.highlight_T()

if __name__ == '__main__':
    root = tk.Tk(); PrimGUI(root); root.mainloop()
